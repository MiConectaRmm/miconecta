  ...
  async generateInstallScript(tenantId: string, configService: ConfigService, format: string) {
    ...
    if (format === 'ps1') {
      const script = [
        ...
        ``,
        `# 1. Download do MSI`,
        `Write-Host "[1/3] Baixando instalador..." -ForegroundColor Yellow`,
        `try {`,
        ...
        `Write-Host "[2/3] Instalando agente..." -ForegroundColor Yellow`,
        `$msiArgs = @("/i", $MsiFile, "/qn", 'SERVER_URL=' + $ServerUrl, 'TENANT_ID=' + $TenantId, 'PROVISION_TOKEN=' + $ProvisionToken)`,
    ...
    const bat = [
      ...
      `  [Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12;^`,
      `  try { Invoke-WebRequest -Uri '%MSI_URL%' -OutFile '%MSI_FILE%' -UseBasicParsing; Write-Host '[OK] Download concluido' }^`,
      ...
      `)`,
      ``,
      `echo [2/3] Instalando agente...`,
      `msiexec /i "%MSI_FILE%" /qn SERVER_URL="%SERVER_URL%" TENANT_ID="%TENANT_ID%" PROVISION_TOKEN="%PROVISION_TOKEN%"`,
      ``,
      `if %errorlevel% neq 0 (`,
